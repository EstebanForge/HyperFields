...
# Update package.json version field (React assets; pattern-based: also repairs a stale value)
if [[ -f "$PROJECT_DIR/package.json" ]]; then
  sedi "s/\"version\": \"[^\"]*\"/\"version\": \"$NEW_VERSION\"/" "$PROJECT_DIR/package.json"
  echo "  ✓ package.json"
fi
...
# only files that actually change are reported.
