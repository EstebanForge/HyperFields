...
The library still initializes normally from such a copy — it does **not** gate boot on web-reachability. `Config::$pluginUrl` is simply empty, and the asset enqueue bails gracefully (`Assets.php` early-returns on an empty URL) instead of emitting a 404; server-side functionality (fields, options pages, export/import) is unaffected. The `bootstrap.php` ABSPATH guard also keeps a root-vendor copy from bootstrapping early in Bedrock's `wp-config` load order. For assets to serve, the library must be loaded from a web-reachable copy.
